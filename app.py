...
with st.container(border=True):
    ...
    if not flagged_site.empty:
        ...
        site_summary = "\n".join([
            f"- {r['Project ID']} ({r['Site Location']}): {r['Reported Days Worked']} days logged, {r['Physical Progress (%)']}% progress" 
            for _, r in flagged_site.iterrows()
        ])
        wa_text = "🚨 *AIVG SITE ALERT*\n\n*Target:* Gosani Block\n" + site_summary
...
with st.container(border=True):
    ...
    if not flagged_payroll.empty:
        ...
        payroll_summary = "\n".join([
            f"- {r['Employee ID']} ({r['Name']}): Disbursed ₹{r['Disbursed Salary (INR)']} vs Sanctioned ₹{r['Sanctioned Salary (INR)']}" 
            for _, r in flagged_payroll.iterrows()
        ])
        wa_text = "🚨 *AIVG PAYROLL BREACH*\n\n*Target:* Gosani Block\n" + payroll_summary
        wa_url = f"https://wa.me/{officer_phone}?text={urllib.parse.quote(wa_text)}"
...
with st.container(border=True):
    ...
    if not flagged_transfers.empty:
        ...
        transfer_summary = "\n".join([
            f"- {r['Transfer ID']} (Emp {r['Employee ID']}): ₹{r['Amount (INR)']} ({r['Transfer Type']})" 
            for _, r in flagged_transfers.iterrows()
        ])
        wa_text = "🚨 *AIVG EXTRA DISBURSAL ALERT*\n\n*Target:* Gosani Block\n" + transfer_summary
        wa_url = f"https://wa.me/{officer_phone}?text={urllib.parse.quote(wa_text)}"
...
with st.container(border=True):
    ...
    if not flagged_welfare.empty:
        ...
        welfare_summary = "\n".join([
            f"- {r['Beneficiary ID']} ({r['Aadhaar Hash']}): Enrolled in {r['Enrolled Count']} schemes" 
            for _, r in flagged_welfare.iterrows()
        ])
        wa_text = "🚨 *AIVG WELFARE FRAUD ALERT*\n\n*Target:* Gosani Block\n" + welfare_summary
        wa_url = f"https://wa.me/{officer_phone}?text={urllib.parse.quote(wa_text)}"
