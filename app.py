...
with col1_side:
    ...
    for idx, row in df1.iterrows():
        ...
        if physical_progress < 50 and attendance_days > 20:
            ...
            st.warning(f"Flagged {row['Worker ID']} ({row['Worker Name']}): High attendance ({attendance_days} days) but progress is under {physical_progress}%.")
            
            # WhatsApp Alert
            msg1 = f"🚨 *AIVG ALERT: WORK PROGRESS ANOMALY*\nWorker: {row['Worker ID']}\nAttendance: {attendance_days} days\nProgress: {physical_progress}%"
            url1 = f"https://api.whatsapp.com/send?phone=919000000000&text={urllib.parse.quote(msg1)}"
            st.link_button("📲 Send WhatsApp Alert", url1, key=f"wa_1_{idx}")
...
with col2_side:
    ...
    if not ghost_records.empty:
        ...
        # WhatsApp Alert
        msg2 = "🚨 *AIVG ALERT: GHOST EMPLOYEE/DUPLICATE ACCOUNT*\nFlagged duplicate bank accounts detected in salary payroll."
        url2 = f"https://api.whatsapp.com/send?phone=919000000000&text={urllib.parse.quote(msg2)}"
        st.link_button("📲 Send WhatsApp Alert", url2, key="wa_2")
...
with col4_side:
    ...
    if not low_dbt.empty:
        ...
        # WhatsApp Alert
        msg4 = "🚨 *AIVG ALERT: SCHEME BENEFICIARY ANOMALY*\nHousing Grant B DBT Success Rate dropped below safe threshold (62.0%)."
...
with col5_side:
    ...
    if not collusion.empty:
        ...
        # WhatsApp Alert
        msg5 = "🚨 *AIVG ALERT: TENDER COLLUSION RISK*\nTender TND_502 flagged for identical bidder IP submission addresses."
