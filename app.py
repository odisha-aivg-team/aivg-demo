...
with st.container(border=True):
    ...
    st.header("🤖 Box 6: AI Risk Analysis Engine")

    st.info(
        "AI-assisted second-level analysis of detected anomalies. "
        "Results are risk indicators and require human verification."
    )
    ...
    if module == "Site Worker & Progress":
        ai_data = edited_site[
            edited_site["AI Status"] != "NORMAL ✅"
        ]

    elif module == "Payroll":
        ai_data = edited_payroll[
            edited_payroll["AI Status"] != "NORMAL ✅"
        ]

    elif module == "Extra Disbursal":
        ai_data = edited_transfers[
            edited_transfers["AI Status"] != "NORMAL ✅"
        ]

    elif module == "Welfare Scheme":
        ai_data = edited_welfare[
            edited_welfare["AI Status"] != "NORMAL ✅"
        ]

    else:
        ai_data = edited_tenders[
            edited_tenders["AI Status"] != "NORMAL ✅"
        ]

    st.write("### 🚨 Records Selected for AI Analysis")

    if ai_data.empty:

        st.success("🟢 No anomalies detected in this module.")
        ...
        if st.button(
            ...
            with st.spinner(
                "🤖 AIVG AI Engine is analyzing the anomalies..."
            ):

                records = ai_data.to_dict(
                    orient="records"
                )
            ...
            st.subheader(
                "🔎 AIVG AI Investigation Report"
            )

            st.markdown(result)

            st.caption(
                "⚠️ AI output is a screening aid only. "
                "It does not establish that corruption or fraud "
                "has occurred. Human investigation is required."
            )
